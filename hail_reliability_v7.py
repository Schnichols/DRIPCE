...
# ─── Page Config ───
st.set_page_config(page_title="Hail Reliability Tool v7", page_icon="🌨️",
...
def compute_costs(df, suffix, replacement_cost, coverage_ratio, annual_premium,
                  ...
                  curve_floor=None, curve_shape=None, reliability_ins=None):
    ...
    reliability_ins (v7): optional separate stow reliability applied to the
    INSURANCE component only (PML blending). Lets insurers price a conservative
    tail case (e.g. P90 of the event-level stow distribution) while developer/
    owner risk (AAL) stays at mean reliability. None = same as `reliability`.
    """
    ...
    R = reliability / 100.0
    R_ins = R if reliability_ins is None else max(reliability_ins, 0.0) / 100.0
    ...
    for angle in ANGLES:
        ...
        pml_eff = R_ins * pml_d + (1 - R_ins) * pml_s   # insurance may price tail reliability
        aal_eff = R * aal_d + (1 - R) * aal_s           # owner risk stays at mean reliability
...
def compute_blended(df20, df32, pct_20, replacement_cost, coverage_ratio, annual_premium,
    ...
    c20 = compute_costs(df20, '20', replacement_cost, coverage_ratio, annual_premium,
                        ...
                        reliability_ins=reliability_ins)
...
if rel_mode == "Single Portfolio":
    ...
    angles_a = _portfolio_angle_select("A — Tracker Angles", "angles_a", [52, 60, 77])
...
# ─── Insurer Tail Reliability (P90) — insurance-only conservative stow case ───
st.sidebar.markdown("### Insurer Tail Reliability")
st.sidebar.caption("Insurers underwrite a conservative tail case of stow reliability, not the "
                   "fleet average. Event-level stow fraction is modeled as a Gaussian around "
                   "each portfolio's mean reliability; the insurance layer (only) is priced at "
                   "the chosen lower percentile. Developer/owner risk stays at mean reliability.")
p90_on = st.sidebar.checkbox("Insurance prices tail reliability", value=False, key="p90_on")
rel_a_ins, rel_b_ins = rel_a, rel_b
p90_conf = 90
if p90_on:
    p90_conf = st.sidebar.selectbox("Confidence Level", [75, 90, 95, 99], index=1,
                                    format_func=lambda c: f"P{c}", key="p90_conf",
                                    help="P90 = 90% confident the stow fraction in any event "
                                         "is at least the priced level.")
    _Z = {75: 0.6745, 90: 1.2816, 95: 1.6449, 99: 2.3263}[p90_conf]
    if rel_mode == "Single Portfolio":
        p90_sigma = st.sidebar.slider(
            "Event Std Dev (%)", 0.0, 10.0, 2.0, 0.25, key="p90_sig",
            help="Spread of per-event stow fraction around the mean. Array field data "
                 "(230 events, 2 below 95%) tail-fits σ ≈ 2.0.")
        rel_a_ins = rel_b_ins = max(stow_reliability - _Z * p90_sigma, 0.0)
        st.sidebar.caption(f"P{p90_conf} insurance reliability: **{rel_a_ins:.2f}%** "
                           f"(owner risk stays at {stow_reliability:.1f}%)")
    else:
        p90_sigma_a = st.sidebar.slider(
            "A — Event Std Dev (%)", 0.0, 10.0, 2.0, 0.25, key="p90_sig_a",
            help="Array field data (230 stow events; only 2 below 95%, min 89%) tail-fits "
                 "σ ≈ 2.0 → P90 ≈ 97.2% at a 99.7% mean. Widen toward 3.75 to weight the "
                 "89% outlier (fat left tail).")
        p90_sigma_b = st.sidebar.slider(
            "B — Event Std Dev (%)", 0.0, 10.0, 4.0, 0.25, key="p90_sig_b",
            help="No public NXT stow-distribution data exists (claims are censored below the "
                 "~5% deductible). Default σ = 4.0 reflects that wider uncertainty → "
                 "P90 ≈ 84.9% at a 90% mean. Update when field data lands.")
        rel_a_ins = max(rel_a - _Z * p90_sigma_a, 0.0)
        rel_b_ins = max(rel_b - _Z * p90_sigma_b, 0.0)
        st.sidebar.caption(f"P{p90_conf} insurance reliability — A: **{rel_a_ins:.2f}%**, "
                           f"B: **{rel_b_ins:.2f}%** (owner risk stays at "
                           f"{rel_a:.1f}% / {rel_b:.1f}%)")
...
def render_portfolio_comparison(compA, compB, relA, relB, stuckA, stuckB, shape_df,
    ...
    if p90_on:
        st.caption(f"🛡️ **Insurer tail reliability active (P{p90_conf}):** insurance is priced at "
                   f"A **{rel_a_ins:.2f}%** / B **{rel_b_ins:.2f}%** stow reliability; "
                   f"developer/owner risk stays at the mean ({relA:.1f}% / {relB:.1f}%).")
    ...
    states_cmp = pdk.Layer("GeoJsonLayer", data=US_STATES_URL, stroked=True, filled=False,
                           pickable=False, get_line_color=[30, 30, 30, 255],
    ...
    def _stacked_alt(stk_df, y_domain=None, height=300):
        """Altair stacked bars matching STACK_COLS/COLORS, with an optional fixed
        y-axis domain so multiple charts can share one scale."""
        order_map = {c: i for i, c in enumerate(STACK_COLS)}
        long = (stk_df.reset_index()
                      .melt(id_vars='Species', var_name='Component', value_name='Cost'))
        long['ord'] = long['Component'].map(order_map)
        y_scale = alt.Scale(domain=[0, y_domain]) if y_domain else alt.Scale()
        return (alt.Chart(long).mark_bar().encode(
                    x=alt.X('Species:N', sort=list(stk_df.index), title=None),
                    y=alt.Y('Cost:Q', stack='zero', scale=y_scale, title='¢/W'),
                    color=alt.Color('Component:N',
                                    scale=alt.Scale(domain=STACK_COLS, range=STACK_COLORS),
                                    legend=alt.Legend(title=None, orient='bottom',
                                                      labelLimit=200)),
                    order=alt.Order('ord:Q', sort='ascending'),
                    tooltip=['Species:N', 'Component:N', alt.Tooltip('Cost:Q', format='.4f')])
    ...
    if comp100 is not None:
        ...
        # ── Site-level stacked bars: Texas Extreme, Kern County, Chicago ──
        hdr_l, hdr_r = st.columns([3, 2])
        with hdr_l:
            st.markdown("##### Cost Composition at Key Sites (¢/W)")
        with hdr_r:
            common_axis = st.toggle(
                "Common Y-axis across sites", value=False, key="site_common_axis",
                help="Off: each site auto-scales to its own range. On: all three sites "
                     "share one Y-axis so the magnitude gap between them is visible.")
        SITES = [("Texas Extreme", 32.6, -100.3),
                 ("Kern County, CA", 35.34, -118.73),
                 ("Chicago, IL", 41.88, -87.63)]

        # Precompute each site's stacked frame + nearest grid point
        site_data = []
        for site_name, s_lat, s_lon in SITES:
            ...
            site_data.append((site_name, li, s_stk))

        gmax = max(float(s_stk[STACK_COLS].sum(axis=1).max()) for _, _, s_stk in site_data)
        y_dom = gmax * 1.05 if common_axis else None

        site_cols = st.columns(len(SITES))
        for sc, (site_name, li, s_stk) in zip(site_cols, site_data):
            with sc:
                st.markdown(f"**{site_name}**")
                st.caption(f"Grid point ({compA['lat'].iloc[li]:.1f}, {compA['lon'].iloc[li]:.1f})")
                st.altair_chart(_stacked_alt(s_stk, y_domain=y_dom, height=300),
                                use_container_width=True)

        # ── Value of stow un-reliability at each key site (the light stack segments) ──
        st.markdown("###### Value of Stow Un-reliability at Key Sites (¢/W)")
        st.caption("The extra cost created by imperfect stow — the lighter segments in the "
                   "stacks above: added insurance and added developer (owner) risk, plus their sum.")
        unrel_rows = []
        for site_name, li, s_stk in site_data:
            for sp in s_stk.index:
                ins_u = float(s_stk.loc[sp, 'Insurance (unreliability)'])
                risk_u = float(s_stk.loc[sp, 'Dev Risk (unreliability)'])
                unrel_rows.append({'Site': site_name, 'Species': sp,
                                   'Insurance (unreliability)': round(ins_u, 4),
                                   'Dev Risk (unreliability)': round(risk_u, 4),
                                   'Total Unreliability': round(ins_u + risk_u, 4)})
        st.dataframe(pd.DataFrame(unrel_rows), hide_index=True, use_container_width=True)
    ...
    with st.expander("📈 Market Share & Dollar Value of Reliability (click to expand)",
        ...
        exp = [{'Section': 'Inputs', 'Key': 'Portfolio A Angles', 'Value': angA_str},
               ...
               {'Section': 'Inputs', 'Key': 'Insurer Tail Reliability', 'Value':
                   (f'P{p90_conf}: A {rel_a_ins:.2f}%, B {rel_b_ins:.2f}%' if p90_on else 'Off')},
...
with p2:
    st.markdown(f"| Parameter | Value |\n|---|---|\n| Annual Premium | **{annual_premium:.2f}%** |"
                ...
                +
                (f"\n| Insurer Tail Rel | **P{p90_conf}: A {rel_a_ins:.2f}%, B {rel_b_ins:.2f}%** |"
                 if p90_on else "\n| Insurer Tail Rel | **Off (mean)** |")
                +
